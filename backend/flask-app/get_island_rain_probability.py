...
def get_island_rain_probability(predicted_data):
    """
    input: 
    ...
    for time in range(0, 31, 5):
        ...
        for point_num in range(len(points_of_interest)):
            ...
            pred_lst.append([point_long, point_lat, time, round(rain_probability_df["rain"].mean(), 1), prob_of_rain])
            

    # convert list of lists into dataframe
    pred_df = pd.DataFrame(pred_lst, columns=["longitude", "latitude", "time", "predicted_rain", "P(predicted_rain > 0)"])
...
def print_example():
    ...
    get_island_rain_probability(sample_data)
